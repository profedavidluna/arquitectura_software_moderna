  ...
  async publish(topic: string, event: { type: string; data: any }): Promise<void> {
    ...
    try {
      await this.producer.send({
        topic,
        messages: [{ key: event.data.orderId || event.data.productId || 'unknown', value: JSON.stringify({ eventType: event.type, timestamp: new Date().toISOString(), source: config.serviceName, data: event.data }) }],
      });
