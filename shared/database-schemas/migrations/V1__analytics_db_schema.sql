-- ============================================
-- Migration: V1__analytics_db_schema.sql
-- Database: analytics_db
-- Description: Initial schema for Analytics Service
-- Created: 2026-11-05
-- ============================================
...
CREATE INDEX idx_pp_product_id ON product_performance(product_id);
CREATE INDEX idx_pp_metric_date ON product_performance(metric_date);
CREATE INDEX idx_pp_product_date ON product_performance(product_id, metric_date DESC);

CREATE INDEX idx_ub_user_id ON user_behavior(user_id);
CREATE INDEX idx_ub_metric_date ON user_behavior(metric_date);
CREATE INDEX idx_ub_user_date ON user_behavior(user_id, metric_date DESC);

CREATE INDEX idx_cohort_date ON cohort_analysis(cohort_date);
...
CREATE TRIGGER trg_pp_updated_at BEFORE UPDATE ON product_performance 
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
CREATE TRIGGER trg_ub_updated_at BEFORE UPDATE ON user_behavior 
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
CREATE TRIGGER trg_cohort_updated_at BEFORE UPDATE ON cohort_analysis 
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
SELECT 
    metric_date, total_orders, total_revenue, average_order_value,
    conversion_rate, cart_abandonment_rate, total_page_views,
    total_sessions, new_users, active_users
FROM metrics WHERE metric_type = 'DAILY_SUMMARY' ORDER BY metric_date DESC;

CREATE OR REPLACE VIEW top_products_by_revenue AS
SELECT 
    product_id, SUM(revenue) AS total_revenue, SUM(purchases) AS total_purchases,
    SUM(units_sold) AS total_units_sold, SUM(views) AS total_views,
    AVG(average_rating)::DECIMAL(3,2) AS avg_rating, SUM(review_count) AS total_reviews,
    CASE WHEN SUM(views) > 0 THEN (SUM(purchases)::DECIMAL / SUM(views))::DECIMAL(5,4) ELSE 0 END AS view_to_purchase_rate
FROM product_performance GROUP BY product_id ORDER BY total_revenue DESC;

CREATE OR REPLACE VIEW user_lifetime_value AS
SELECT 
    user_id, COUNT(DISTINCT metric_date) AS active_days,
    SUM(total_orders) AS lifetime_orders, SUM(total_spent) AS lifetime_value,
    AVG(average_order_value)::DECIMAL(10,2) AS avg_order_value,
    MIN(metric_date) AS first_activity, MAX(metric_date) AS last_activity
FROM user_behavior GROUP BY user_id ORDER BY lifetime_value DESC;
...
SELECT 
    ...
    revenue_day_30, revenue_day_90
FROM cohort_analysis ORDER BY cohort_date DESC;
...
BEGIN
    DELETE FROM events WHERE created_at < CURRENT_DATE - (retention_days || ' days')::INTERVAL;
...
INSERT INTO schema_migrations (migration_name) VALUES ('V1__analytics_db_schema');
